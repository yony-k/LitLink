...
public class NoteService {
  ...
  public void saveNote(long memberId, CreateNoteDTO createNoteDTO) {
    // 북마크 가져오기
    BookMark bookMark = bookMarkService.findBookMark(memberId, createNoteDTO.bookMarkId());
    // 노트 생성 후 저장
    Note note =
        Note.builder()
            .bookmark(bookMark)
  ...
  public List<NoteDTO> getNotes(long memberId, long bookmarkId) {
    // 노트 소유주 확인
    BookMark findBookMark = bookMarkService.findBookMark(memberId, bookmarkId);
    if (findBookMark.getMember().getMemberId() != memberId) {
      throw new CustomException(NoteExceptionType.NOTE_NOT_FOUND);
    }
